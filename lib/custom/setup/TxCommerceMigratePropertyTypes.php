...
/**
 * Migrates property types from TYPO3 commerce extension
 */
class TxCommerceMigratePropertyTypes extends TxCommerceBase
{
	private $map = array();
	private $size = 100;
	...
	private $sql = '
		...
		WHERE "deleted" = 0 AND "has_valuelist" = 0
		ORDER BY "uid"
		LIMIT ? OFFSET ?
	';
	...
	{
		$this->msg( 'TYPO3 commerce: Migrate property types', 0 ); $this->status( '' );
	...
	{
		$map = array();
		$manager = \Aimeos\MShop\Factory::createManager( $this->additional, 'product/property/type' );

		$search = $manager->createSearch();
		$search->setConditions( $search->compare( '==', 'product.property.type.code', array_keys( $list ) ) );
